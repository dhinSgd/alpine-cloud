...
# 必备 cmdline 参数：
#   modules=...,btrfs       让 Alpine initramfs 主动 modprobe btrfs
#   rootfstype=btrfs        否则 initramfs init 默认按 ext3/ext4 尝试挂载（subvol 参数会被 ext4 拒绝）
#   rootflags=subvol=@      启用 @ 子卷
#   console=ttyS0,115200    云环境串口可见（boot test/阿里云控制台都依赖）
GRUB_DEFAULT_FILE="$MOUNT_DIR/etc/default/grub"
ensure_dir "$(dirname "$GRUB_DEFAULT_FILE")"
log_info "写入 /etc/default/grub（GRUB_CMDLINE_LINUX_DEFAULT 含 btrfs 必要参数）"
cat > "$GRUB_DEFAULT_FILE" <<'GRUB_EOF'
# Generated by slim-alpine build
...
for cfg in "${CANDIDATES[@]}"; do
  ...
  # rootfstype=ext4 -> btrfs（如果 grub-mkconfig 写错了）
  sed -i -E 's|rootfstype=ext4|rootfstype=btrfs|g' "$cfg"
  # 兜底：如果根本没出现 rootfstype=，追加 rootfstype=btrfs
  if ! grep -qE 'rootfstype=' "$cfg"; then
    sed -i -E "s|(root=UUID=[A-Fa-f0-9-]+)|\1 rootfstype=btrfs|g" "$cfg"
  fi
